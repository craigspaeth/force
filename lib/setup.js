...
export default function (app) {
  ...
  // Add stubbed API servers and a stubbed user for testing
  // TODO: https://github.com/artsy/force/issues/1149
  if (NODE_ENV === 'test') {
    app.use((req, res, next) => {
      if (!req.query['test-login']) { return next() }
    ...
    app.use('/__positron', require('antigravity').server)
  }
  ...
  app.use((req, res, next) => {
    ...
    if (isPhone) require('../mobile')(req, res, next)
    else next()
  })
